...
export function renderLetter(template, data = {}) {
  ...
  // Recipient block
  let recipientBlock;
  if (template.recipient === 'bureau') {
    const bureau = d.bureau && BUREAU_ADDRESSES[d.bureau] ? d.bureau : 'Experian';
    recipientBlock = BUREAU_ADDRESSES[bureau];
    d.bureau = bureau;
  } else {
    recipientBlock = `${d.creditor || d.recipientName || '[COMPANY NAME]'}\n${d.recipientAddress || '[COMPANY MAILING ADDRESS]'}`;
  }

  // Sender identity lines (only those provided)
  const idLines = [];
  if (d.dob)      idLines.push(`Date of Birth: ${d.dob}`);
  if (d.ssnLast4) idLines.push(`SSN (last 4): ${d.ssnLast4}`);

  const senderBlock = [
    d.fullName || '[YOUR FULL NAME]',
    d.address || '[YOUR STREET ADDRESS]',
    d.cityStateZip || '[CITY, STATE ZIP]',
    ...idLines,
  ].join('\n');

  const body = (template.paragraphs || []).map(p => applyTokens(p, d)).join('\n\n');
  const reLine = applyTokens(template.reLine || template.label, { ...d, date: today });
  const demand = template.demand ? applyTokens(template.demand, d) : '';
  const enclosures = (template.suggestedDocuments || []).filter(x => !/^none/i.test(x));

  const salutation = template.recipient === 'bureau'
  ...
  return [
    ...
    demand ? `\n${demand}` : '',
    '',
    'Please complete your investigation within the time required by law and send me written confirmation of the results.',
    '',
    'Sincerely,',
    '',
    '',
