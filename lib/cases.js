...
async function listTemplatesForAdmin({ includeInactive = false } = {}) {
  ...
  const { data, error } = await withTimeout(query, 8000, 'Dispute template lookup timed out.');
  if (error) {
...
async function listFollowupRulesForAdmin({ includeDisabled = false } = {}) {
  ...
  const { data, error } = await withTimeout(query, 8000, 'Follow-up rule lookup timed out.');
  if (error) {
    if (isMissingSchemaError(error)) return [];
    throw error;
  }
