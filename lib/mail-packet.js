...
async function addTextPage(pdf, { title, subtitle, body, footer, accent, pdfLib }) {
  const { StandardFonts, rgb } = pdfLib;
  const accentColor = accent || rgb(0.06, 0.73, 0.51);
  ...
  page.drawText('CREDITOS', { x: margin, y: height - 52, size: 18, font: titleFont, color: accentColor });
  ...
  page.drawLine({ start: { x: margin, y: height - 66 }, end: { x: width - margin, y: height - 66 }, thickness: 1.2, color: accentColor });
...
async function embedDocumentPages(pdf, { bytes, mimeType, pdfLib }) {
...
export async function createCertifiedMailPacket({
  ...
  for (const doc of supportDocs) {
    await embedDocumentPages(pdf, { ...doc, pdfLib });
  }
