...
function hasUsefulMoney(value) {
  return value != null && cleanText(value).length > 0 && cleanText(value).toLowerCase() !== 'unknown';
}

function hasMaskedAccountHint(value) {
  const text = cleanText(value, 80);
  if (!text) return false;
  return /(?:\*{2,}\d{2,6}|\d{4,})/.test(text);
}

function isGenericCreditor(value = '') {
...
function normalizeTypeHint(value = '') {
  const normalized = normalizedLookup(value).replace(/[_-]/g, ' ');
  if (normalized.includes('collection')) return 'collection';
  if (normalized.includes('charge')) return 'charge off';
  if (normalized.includes('late')) return 'late payment';
  if (normalized.includes('student')) return 'student loan';
  ...
  if (normalized.includes('outdated') || normalized.includes('obsolete')) return 'outdated';
...
function matchAccountFromReport(item = {}, accounts = []) {
  const itemCreditor = normalizedLookup(item.creditor);
  const itemAccount = maskAccountNumber(item.account_number || item.account || '');
  const itemBureau = normalizedLookup(item.bureau);
  const itemType = normalizeTypeHint(item.account_type || item.type);
  ...
  for (const account of accounts) {
    let score = 0;
    const accountCreditor = normalizedLookup(account.creditor_name);
    const accountMask = maskAccountNumber(account.account_number_masked || '');
    const accountBureau = normalizedLookup(account.bureau);
    const accountType = normalizeTypeHint(account.account_type);

    if (itemAccount && accountMask && itemAccount === accountMask) score += 6;
    if (itemCreditor && accountCreditor && itemCreditor === accountCreditor) score += 5;
    else if (itemCreditor && accountCreditor && (itemCreditor.includes(accountCreditor) || accountCreditor.includes(itemCreditor))) score += 3;
    if (itemBureau && accountBureau && itemBureau === accountBureau) score += 2;
    if (itemType && accountType && itemType === accountType) score += 2;
    if (itemType === 'collection' && account.is_collection) score += 2;
    if (itemType === 'charge off' && account.is_charge_off) score += 2;
    if (itemType === 'student loan' && account.is_student_loan) score += 2;
    if (itemType === 'late payment' && (account.late_payments?.length || /late|delinquen|past due/i.test(`${account.payment_status} ${account.payment_history}`))) score += 3;
    if (itemType === 'balance' && account.balance != null) score += 1;
...
function splitAccountBlocks(text) {
  ...
  const accountStart = '(?:account name|creditor|furnisher|subscriber(?: name)?|collection agency)';
  const blockPattern = new RegExp(`(?:^|\\n)((${accountStart})\\b[\\s\\S]*?)(?=\\n${accountStart}\\b|$)`, 'gi');
  const pieces = [...raw.matchAll(blockPattern)].map(match => match[1].trim()).filter(item => item.length > 80);
  ...
  for (const line of lines) {
    const startsAccount = /(?:account name|creditor|furnisher|subscriber(?: name)?|collection agency)\b/i.test(line);
...
function normalizeAccount(block, index, defaultBureau) {
  ...
  const accountNumber = valueAfter(block, ['account number', 'account #', 'acct #'], 80);
  ...
  const disputes = aiDisputes.length
    ? aiDisputes.map((item, index) => {
        const matching = intelligence.disputes.find(candidate =>
          normalizedLookup(candidate.creditor) === normalizedLookup(item.creditor)
          || (candidate.account_number && item.account_number && candidate.account_number === maskAccountNumber(item.account_number))
        );
        const matchedAccount = matchAccountFromReport(item, intelligence.normalized_report.accounts || []);
        const resolvedCreditor = !isGenericCreditor(item.creditor)
          ? cleanText(item.creditor, 150)
          : matching?.creditor || matchedAccount?.creditor_name || 'Review report item';
        const resolvedAccountNumber = maskAccountNumber(
          (hasMaskedAccountHint(item.account_number) && item.account_number) ||
          (hasMaskedAccountHint(item.account) && item.account) ||
          matching?.account_number ||
          matchedAccount?.account_number_masked ||
          ''
        );
        const resolvedAccountType = cleanText(
          item.account_type ||
        ...
        return {
          ...item,
          creditor: resolvedCreditor,
          account: resolvedAccountNumber || cleanText(item.account || matching?.account || '', 80) || 'Review report',
          account_number: resolvedAccountNumber,
          account_type: resolvedAccountType,
          balance: hasUsefulMoney(item.balance) ? item.balance : (matching?.balance || moneyText(matchedAccount?.balance) || null),
          credit_limit: hasUsefulMoney(item.credit_limit) ? item.credit_limit : (matching?.credit_limit || moneyText(matchedAccount?.credit_limit) || null),
          payment_status: hasUsefulValue(item.payment_status) ? item.payment_status : (matching?.payment_status || matchedAccount?.payment_status || matchedAccount?.account_status || 'Unknown'),
          late_payments: hasUsefulValue(item.late_payments) ? item.late_payments : (matching?.late_payments || matchedAccount?.late_payments?.join(', ') || null),
          opened_date: item.opened_date || matching?.opened_date || matchedAccount?.date_opened || null,
          reported_date: item.reported_date || matching?.reported_date || matchedAccount?.date_reported || null,
          first_delinquency_date: item.first_delinquency_date || matching?.first_delinquency_date || matchedAccount?.date_of_first_delinquency || null,
          bureau: item.bureau || matching?.bureau || matchedAccount?.bureau || intelligence.normalized_report.bureau || 'All',
          charge_off: typeof item.charge_off === 'boolean' ? item.charge_off : Boolean(matching?.charge_off ?? matchedAccount?.is_charge_off),
          collection: typeof item.collection === 'boolean' ? item.collection : Boolean(matching?.collection ?? matchedAccount?.is_collection),
