...
export async function isLaunchVerificationAdmin(userId) {
  ...
  const { data, error } = await withTimeout(supabaseAdmin
    ...
    .eq('id', userId)
    .maybeSingle(), 8000, 'Launch admin lookup timed out.');
  ...
  if (error) {
    if (isMissingSchemaError(error)) {
      const fallback = await withTimeout(
        ...
        8000,
        'Launch admin email lookup timed out.'
      ).catch(() => ({ data: null, error: null }));
      return isLaunchVerificationAdminRow(fallback?.data);
    }
    throw error;
  }
...
export async function loadLaunchVerificationDashboard(userId) {
  ...
  const { data, error } = await withTimeout(supabaseAdmin
    ...
    .limit(200), 8000, 'Launch verification dashboard lookup timed out.');

  if (error) {
    if (isMissingSchemaError(error)) {
      return {
        admin: true,
        ...summarizeLaunchVerificationEvents([])
      };
    }
    throw error;
  }
