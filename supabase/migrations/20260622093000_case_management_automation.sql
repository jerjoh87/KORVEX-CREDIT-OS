...
-- Deadline alerts table (generated by the follow-up engine or manually)
create table if not exists public.deadline_alerts (
  ...
  alert_type   text not null default 'deadline'
                check (alert_type in ('deadline','followup','escalation','custom')),
  alert_date   timestamptz not null,
  title        text not null,
  body         text,
  status       text not null default 'pending'
                check (status in ('pending','read','dismissed','sent')),
  metadata     jsonb not null default '{}'::jsonb,
  created_at   timestamptz not null default now(),
  updated_at   timestamptz not null default now()
);
...
create policy "deadline_alerts_select_own" on public.deadline_alerts
  for select to authenticated using ((select auth.uid()) = user_id);
drop policy if exists "deadline_alerts_insert_own" on public.deadline_alerts;
create policy "deadline_alerts_insert_own" on public.deadline_alerts
  for insert to authenticated with check ((select auth.uid()) = user_id);
drop policy if exists "deadline_alerts_update_own" on public.deadline_alerts;
create policy "deadline_alerts_update_own" on public.deadline_alerts
  for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);

create index if not exists deadline_alerts_user_date_idx on public.deadline_alerts (user_id, alert_date, status);
create index if not exists deadline_alerts_case_idx on public.deadline_alerts (case_id);
